...
class FetchImages:
    def __init__(
        ...
        self.size_km = size_km // 2 # due to the fact that, original value represent half of box edge len
    ...
    def _define_time_interval(self, row):
        return (row["Date_standard"] - timedelta(days=self.time_interval_size), row["Date_standard"] + timedelta(days=self.time_interval_size))

    def _define_bbox(self, latitude: float, longitude: float) -> Tuple[BBox, Tuple[int]]:
        ...
        bbox_WGS84 = [lat_min, lon_min, lat_max, lon_max]
        bbox = BBox(bbox=bbox_WGS84, crs=CRS.WGS84)
        size = bbox_to_dimensions(bbox, resolution=self.resolution)
    
        if self.verbose:
            print("To see actual bbox go to this page http://bboxfinder.com/#0.000000,0.000000,0.000000,0.000000 and paste cords")
    ...
    def rename_request_png_based_on_json(self):
        for subdir, dirs, files in os.walk(self.save_dir):
            if 'request.json' in files and 'response.png' in files:  # Check both files exist
                ...
                try:
                    ...
                    with open(json_file_path, 'r') as json_file:
                        ...
                        value = tuple(data["request"]["payload"]["input"]["bounds"]["bbox"])
                        
                        if value and value in self.mapper:
                            # Get the new file name from the mapping
                            new_file_name = str(self.mapper[value])
                            new_png_file_path = os.path.join(subdir, f"{new_file_name}.png")
                            
                            # Retry logic to rename the file if it's locked
                            retries = 3
                            for attempt in range(retries):
                                try:
                                    shutil.move(png_file_path, new_png_file_path)  # Rename the PNG file
                                    print(f'Renamed {png_file_path} to {new_png_file_path}')
                                    ...
                                    if attempt < retries - 1:
                                        ...
                                        print(f"Failed to rename {png_file_path} after {retries} attempts.")
